...
public class PlayerControl : MonoBehaviour {
    ...
    void PowerUP(){
        ...
        if(playerLength < 4 & isPowerUp) {
            playerLength += 10 * Time.deltaTime;
            this.transform.localScale = new Vector2 (1,playerLength);
            StartCoroutine(PowerDown());
        }

        else if (playerLength > 1 & !isPowerUp){
            playerLength -= 20 * Time.deltaTime;
            this.transform.localScale = new Vector2 (1,playerLength);
        }
    }
